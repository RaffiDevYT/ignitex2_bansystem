...
AddEventHandler('esx:playerLoaded', function(source)
    ...
    if isBypassing2 then
        ...
        BanNewAccount(tonumber(source), "Tried To Bypass ignitex2_BanSystem", os.time() + (300 * 86400))
	    DropPlayer(source, "kick reason: you were banned from server")
    end
end)
...
AddEventHandler('Initiate:BanSql', function(hex, id, reason, name, day)
    ...
    MySQL.Async.execute('UPDATE ignitex2_bansystem SET Reason = @Reason, isBanned = @isBanned, Expire = @Expire WHERE Steam = @Steam', 
    {
        ['@isBanned'] = 1,
        ['@Reason'] = reason,
        ['@Steam'] = hex,
...
AddEventHandler('TargetPlayerIsOffline', function(hex, reason, xAdmin, day)
    ...
    MySQL.Async.fetchAll('SELECT Steam FROM ignitex2_bansystem WHERE Steam = @Steam',
    {
        ['@Steam'] = hex
...
function InitiateDatabase(source)
    ...
    MySQL.Async.fetchAll('SELECT * FROM ignitex2_bansystem WHERE Steam = @Steam',
    {
        ['@Steam'] = ST

    }, function(data) 
        if data[1] == nil then
            MySQL.Async.execute('INSERT INTO ignitex2_bansystem (Steam, License, Tokens, Discord, IP, Xbox, Live, Reason, Expire, isBanned) VALUES (@Steam, @License, @Tokens, @Discord, @IP, @Xbox, @Live, @Reason, @Expire, @isBanned)',
            {
                ['@Steam'] = ST,
                ['@License'] = LC,
                ['@Discord'] = DS,
...
RegisterServerEvent("ignitex2_BanSystem:BanMe")
AddEventHandler("ignitex2_BanSystem:BanMe", function(Reason, Time)
    local source = source
    for k, v in ipairs(GetPlayerIdentifiers(source)) do
        if string.sub(v, 1, string.len("steam:")) == "steam:" then
            Cheat = v
...
RegisterServerEvent("ignitex2_BanSystem:CheckBan")
AddEventHandler("ignitex2_BanSystem:CheckBan", function(hex)
    local source = source
    MySQL.Async.fetchAll('SELECT * FROM ignitex2_bansystem WHERE Steam = @Steam',
    {
        ['@Steam'] = hex
...
function ReloadBans()
    Citizen.CreateThread(function()
        BannedAccounts = {}
        MySQL.Async.fetchAll('SELECT * FROM ignitex2_bansystem', {}, function(info)
            for i = 1, #info do
